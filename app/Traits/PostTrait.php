...
trait PostTrait
{
    ...
    public function storePostThumbAndGetFileName($image)
    {
        ...
        Image::make($image)->resize(736, 736 / $ratio)->save($this->pathImage() . '/' . $fileName);
        Image::make($image)->resize(356, 356 / $ratio)->save($this->pathImage() . '/356/' . $fileName);
        Image::make($image)->resize(300, 300 / $ratio)->save($this->pathImage() . '/300/' . $fileName);
        Image::make($image)->resize(80, 80 / $ratio)->save($this->pathImage() . '/80/' . $fileName);
